...
The document shows the count of each keyword in the pages selected by the user
...
The Excel file will have tabs named after each keyword in the .txt file, each tab will contain all the comments that the keywords are found in. The corresponding url is also provided for each comment, which was very important for the client, as they needed context for each comment. 
